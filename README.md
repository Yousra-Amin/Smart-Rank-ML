...
├── requirements.txt
├── 01_download_data.py        ← Badhon: Download from Hugging Face (internet needed)
├── step1_offline_fallback.py     ← Badhon: Use this if HuggingFace fails (no internet)
├── 02_clean_data.py           ← Badhon: Clean text
├── 03_feature_engineering.py  ← Yousra: Build 12 features (TF-IDF, BM25, CTR, etc.)
├── 04_train_model.py          ← Ovi:    Train LambdaMART (XGBoost rank:ndcg)
